...
func TestOrdinalFormatting(t *testing.T) {
	...
	// Test day ordinals
	utils.AssertEqual(t, "1st", idfs.Format(date1st, "dt"))
	utils.AssertEqual(t, "2nd", idfs.Format(date2nd, "dt"))
	utils.AssertEqual(t, "3rd", idfs.Format(date3rd, "dt"))
	utils.AssertEqual(t, "21st", idfs.Format(date21st, "dt"))

	// Test month ordinals (this was the bug - was using day value for month ordinals)
	utils.AssertEqual(t, "1st", idfs.Format(date1st, "mt"))   // January = 1st
	utils.AssertEqual(t, "2nd", idfs.Format(date2nd, "mt"))   // February = 2nd
	utils.AssertEqual(t, "3rd", idfs.Format(date3rd, "mt"))   // March = 3rd
	utils.AssertEqual(t, "11th", idfs.Format(date21st, "mt")) // November = 11th (not 21st)
}
...
func TestOrdinalEdgeCases(t *testing.T) {
	...
	// Day ordinals
	utils.AssertEqual(t, "11th", idfs.Format(date11th, "dt"))
	utils.AssertEqual(t, "12th", idfs.Format(date12th, "dt"))
	utils.AssertEqual(t, "13th", idfs.Format(date13th, "dt"))
	utils.AssertEqual(t, "22nd", idfs.Format(date22nd, "dt"))
	utils.AssertEqual(t, "23rd", idfs.Format(date23rd, "dt"))

	// Month ordinals
	utils.AssertEqual(t, "11th", idfs.Format(date11th, "mt")) // November
	utils.AssertEqual(t, "12th", idfs.Format(date12th, "mt")) // December
}
