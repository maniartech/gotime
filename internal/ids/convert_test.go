...
func TestConvertLayoutA(t *testing.T) {
	// c := ids.Format(time.Now(), "dd-mm-yyyyThhh:ii:ss.000000000000 zzoo")
...
func TestTZ(t *testing.T) {
	date := ids.Format(time.Now(), "yyyy-mm-ddThh:ii:aa:ss.000000000TZZ")
	fmt.Println(date)
}

func TestConvert(t *testing.T) {
	date, err := ids.Convert("2012-Jun-03 00:00:00.123", "yyyy-mmm-dd hhh:ii:ss.999", "yyyy-mm-dd hh:ii:aa:ss")
...
func BenchmarkConvert(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		ids.Convert("01/24/1984", "dd/mm/yyyy", "yyyy/dd/mm")
	}
}
